...
function insertJs(name) {
  ...
  const templateIndexHandle = Handlebars.compile(componentIndexTpl);
  const templateVueHandle = Handlebars.compile(componentVueTpl)
  const context = { upperComponentName, name };

  fs.writeFileSync(`${componentDir}/index.js`, `${templateIndexHandle(context)}`);

  fs.writeFileSync(`${componentSrcDir}/${name}.vue`, `${templateVueHandle(context)}`);
}
